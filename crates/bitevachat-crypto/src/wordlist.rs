//! Embedded BIP39 English 2048-word list.
//!
//! The wordlist is sorted lexicographically as required by the BIP39
//! specification. Lookup is performed via binary search for O(log n)
//! performance.
//!
//! Reference: <https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt>

/// Complete BIP39 English wordlist (2048 words, sorted).
...
/// Looks up a word in the BIP39 wordlist and returns its 0-based index.
///
/// Uses binary search over the sorted wordlist. Returns `None` if the
...
/// Returns the word at the given 0-based index (0\u20132047).
///
...
mod tests {
    ...
    #[test]
    fn word_to_index_not_found() {
        assert_eq!(word_to_index("notaword"), None);
        assert_eq!(word_to_index(""), None);
    }

    #[test]
    fn index_to_word_known() {
        assert_eq!(index_to_word(0), Some("abandon"));
        assert_eq!(index_to_word(2047), Some("zoo"));
    }

    #[test]
    fn index_to_word_out_of_range() {
    ...
    fn roundtrip_all_words() {
        for (i, word) in WORDLIST.iter().enumerate() {
            ...
            assert_eq!(idx, Some(i as u16), "roundtrip failed for \'{word}\' at index {i}");
            let w = index_to_word(i as u16);
            assert_eq!(w, Some(*word));
        }
    }
}