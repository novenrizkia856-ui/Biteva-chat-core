...
fn ed25519_rfc8032_test1_sign_verify() -> std::result::Result<(), BitevachatError> {
    // RFC 8032 TEST 1 seed (private key).
    let seed: [u8; 32] = [
    ...
    // Determinism: same seed always produces the same keypair.
    let kp2 = Keypair::from_seed(&seed);
    assert_eq!(pk.as_bytes(), kp2.public_key().as_bytes());

    // Sign empty message (RFC 8032 Test 1 message is empty).
    let sig = kp.sign(b"");

    // Determinism: same key + message always produces the same signature.
    let sig2 = kp2.sign(b"");
