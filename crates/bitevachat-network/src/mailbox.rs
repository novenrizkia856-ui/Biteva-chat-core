...
mod tests {
    ...
    /// Creates a dummy WireMessage for testing.
    fn make_wire(recipient_byte: u8) -> WireMessage {
        let msg = Message {
            sender: Address::new([0xAA; 32]),
            recipient: Address::new([recipient_byte; 32]),
            payload_type: PayloadType::Text,
            payload_ciphertext: b"test-payload".to_vec(),
            node_id: NodeId::new([0x01; 32]),
            nonce: Nonce::new([0x00; 12]),
            timestamp: Timestamp::now(),
            message_id: MessageId::new([recipient_byte; 32]),
        };

        let signature = bitevachat_types::Signature::from_bytes([0x00; 64]);
    ...
    fn store_and_drain() {
        ...
        assert_eq!(mailbox.pending_count_for(&recipient), 0);

        assert!(mailbox.store(&recipient, make_wire(0xBB)));
        assert!(mailbox.store(&recipient, make_wire(0xBB)));

        assert_eq!(mailbox.total_count(), 2);
        assert_eq!(mailbox.pending_count_for(&recipient), 2);

        let messages = mailbox.drain(&recipient);
    ...
    fn drain_nonexistent_returns_empty() {
        let mut mailbox = Mailbox::new();
        let messages = mailbox.drain(&[0xFF; 32]);
        assert!(messages.is_empty());
    }

    #[test]
    fn per_recipient_fifo_eviction() {
        let mut mailbox = Mailbox::with_limits(2, 100, 3600);
        let recipient = [0xCC; 32];

        assert!(mailbox.store(&recipient, make_wire(0x01)));
        assert!(mailbox.store(&recipient, make_wire(0x02)));
        assert_eq!(mailbox.pending_count_for(&recipient), 2);

        // Third store evicts the oldest.
        assert!(mailbox.store(&recipient, make_wire(0x03)));
        assert_eq!(mailbox.pending_count_for(&recipient), 2);
        assert_eq!(mailbox.total_count(), 2);
    }
    ...
    fn global_limit_rejects() {
        let mut mailbox = Mailbox::with_limits(100, 3, 3600);

        let r1 = [0x01; 32];
        let r2 = [0x02; 32];

        assert!(mailbox.store(&r1, make_wire(0x01)));
        assert!(mailbox.store(&r1, make_wire(0x01)));
        assert!(mailbox.store(&r2, make_wire(0x02)));

        // Global limit reached.
        assert!(!mailbox.store(&r2, make_wire(0x02)));
        assert_eq!(mailbox.total_count(), 3);
    }

    #[test]
    fn purge_expired_removes_old_entries() {
        let mut mailbox = Mailbox::with_limits(100, 100, 60);
        let recipient = [0xDD; 32];

        // Store a message.
        assert!(mailbox.store(&recipient, make_wire(0xDD)));
        assert_eq!(mailbox.total_count(), 1);

        // Manually expire it by replacing the entry with an old timestamp.
        if let Some(queue) = mailbox.queues.get_mut(&recipient) {
            if let Some(entry) = queue.front_mut() {
                // Set stored_at to 2 minutes ago (TTL is 60s).
    ...
    fn stats_returns_correct_values() {
        ...
        mailbox.store(&r1, make_wire(0x01));
        mailbox.store(&r1, make_wire(0x01));
        mailbox.store(&r2, make_wire(0x02));
    ...
    #[test]
    fn with_limits_clamps_minimums() {
        let mailbox = Mailbox::with_limits(0, 0, 10);
        assert_eq!(mailbox.max_per_recipient, 1);
        assert_eq!(mailbox.max_total, 1);
        assert_eq!(mailbox.ttl, Duration::from_secs(60));
    }
    ...
    fn drain_skips_expired() {
        ...
        mailbox.store(&recipient, make_wire(0xEE));
        mailbox.store(&recipient, make_wire(0xEE));
        ...
        let messages = mailbox.drain(&recipient);
        assert_eq!(messages.len(), 1); // Only the non-expired one.
        assert!(mailbox.is_empty());
    }
    ...
    fn multiple_recipients_independent() {
        ...
        let m1 = mailbox.drain(&r1);
        assert_eq!(m1.len(), 1);
        assert_eq!(mailbox.total_count(), 2);

        let m2 = mailbox.drain(&r2);
        assert_eq!(m2.len(), 2);
        assert!(mailbox.is_empty());
    }
}