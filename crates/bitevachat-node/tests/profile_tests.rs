...
pub struct AppConfig {
    ...
    ///
    /// Cached profiles expire after this duration and must be
    /// re-fetched from storage or gossip.
    pub profile_cache_ttl_secs: u64,

    /// Maximum avatar blob size in bytes.
    ///
    /// Avatars larger than this are rejected. Default: 1 MiB.
...
impl AppConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.profile_cache_ttl_secs == 0 {
            return Err(BitevachatError::ConfigError {
                reason: "profile_cache_ttl_secs must be greater than 0".into(),
            });
        }

        if self.max_avatar_size == 0 {
            return Err(BitevachatError::ConfigError {
                reason: "max_avatar_size must be greater than 0".into(),
            });
        }
...
mod tests {
    ...
    fn default_values_match_spec() {
        ...
        assert_eq!(config.max_avatar_size, 1_048_576);
    }
    ...
    #[test]
    fn zero_profile_cache_ttl_rejected() {
        let config = AppConfig {
    ...
    #[test]
    fn zero_max_avatar_size_rejected() {
        let config = AppConfig {
            max_avatar_size: 0,
            ..AppConfig::default()
        };
    ...
    fn config_serde_roundtrip() -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(config.profile_cache_ttl_secs, parsed.profile_cache_ttl_secs);
        assert_eq!(config.max_avatar_size, parsed.max_avatar_size);
        Ok(())
    }
}